...
namespace Features
{
    ...
    SkillType KitBuffBot::ResolveSkillItemName(const char* itemname)
    {
        ...
        char lc_itemname[SIZE_MAX_ITEM_NAME];
        strcpy_s(lc_itemname, SIZE_MAX_ITEM_NAME, itemname);

        for(auto& c : lc_itemname) {
            c = tolower(c);
        }

        if (strstr(lc_itemname, "raging evasion"))         return SkillType::Raging_Evasion;
        if (strstr(lc_itemname, "raging defense"))         return SkillType::Raging_Defense;
        if (strstr(lc_itemname, "raging fire"))            return SkillType::Raging_Fire;
        if (strstr(lc_itemname, "siege defense"))          return SkillType::Siege_Defense;
        if (strstr(lc_itemname, "turnaround"))             return SkillType::Turnaround;
        if (strstr(lc_itemname, "back move mach"))         return SkillType::Back_Move_Mach;
        if (strstr(lc_itemname, "defense up"))             return SkillType::Defense_Up;
        if (strstr(lc_itemname, "concentration"))          return SkillType::Concentration;
        if (strstr(lc_itemname, "frenzy"))                 return SkillType::Frenzy;
        if (strstr(lc_itemname, "berserker"))              return SkillType::Berserker;
        if (strstr(lc_itemname, "silence"))                return SkillType::Silence;
        if (strstr(lc_itemname, "purchasing shop"))        return SkillType::Purchasing_Shop;
        if (strstr(lc_itemname, "selling shop"))           return SkillType::Selling_Shop;
        if (strstr(lc_itemname, "hyper moving"))           return SkillType::Hyper_Moving;
        if (strstr(lc_itemname, "chain rolling"))          return SkillType::Chain_Rolling;
        if (strstr(lc_itemname, "overbooster"))            return SkillType::Overbooster;
        if (strstr(lc_itemname, "evasion"))                return SkillType::Evasion_Up;
        if (strstr(lc_itemname, "missile shot"))           return SkillType::Missile_Shot;
        if (strstr(lc_itemname, "fire shot"))              return SkillType::Fire_Shot;
        if (strstr(lc_itemname, "hyper shot"))             return SkillType::Hyper_Shot;
        if (strstr(lc_itemname, "shield paralyze"))        return SkillType::Shield_Paralyze;
        if (strstr(lc_itemname, "camouflage"))             return SkillType::Camouflage;
        if (strstr(lc_itemname, "snare shot"))             return SkillType::Snare_Shot;
        if (strstr(lc_itemname, "ground accelerator"))     return SkillType::Ground_Accelerator;
        if (strstr(lc_itemname, "barrier"))                return SkillType::Barrier;
        if (strstr(lc_itemname, "remedy"))                 return SkillType::Remedy;
        if (strstr(lc_itemname, "collision absorption"))   return SkillType::Collision_Absorption;
        if (strstr(lc_itemname, "air siege"))              return SkillType::Air_Siege_Mode;
        if (strstr(lc_itemname, "siege mode"))             return SkillType::Siege_Mode;
        if (strstr(lc_itemname, "purify"))                 return SkillType::Purify;
        if (strstr(lc_itemname, "full recovery"))          return SkillType::Full_Recovery;
        if (strstr(lc_itemname, "scanning"))               return SkillType::Scanning;
        if (strstr(lc_itemname, "call of hero"))           return SkillType::Call_of_Hero;
        if (strstr(lc_itemname, "smart sp"))               return SkillType::Smart_SP;  
        if (strstr(lc_itemname, "fuel replenish"))         return SkillType::Fuel_Replenish;
        if (strstr(lc_itemname, "reverse engine"))         return SkillType::Reverse_Engine;
        ...
        if (strstr(lc_itemname, "air bombing mode"))       return SkillType::Air_Bombing_Mode;
        if (strstr(lc_itemname, "charge shot"))            return SkillType::Charge_Shot;
        if (strstr(lc_itemname, "healing field"))          return SkillType::Healing_Field;
        if (strstr(lc_itemname, "elevation"))              return SkillType::Elevation;
        if (strstr(lc_itemname, "big boom"))               return SkillType::Big_Boom;
        if (strstr(lc_itemname, "invisible"))              return SkillType::Invisible;
        if (strstr(lc_itemname, "deploy chaffs"))          return SkillType::Deploy_Chaffs;
        if (strstr(lc_itemname, "multitarget mode"))       return SkillType::Multitarget_Mode;
        if (strstr(lc_itemname, "ground bombing mode"))    return SkillType::Ground_Bombing_Mode;
        return SkillType::Unknown;
    }
